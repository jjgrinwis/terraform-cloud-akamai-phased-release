...
resource "akamai_cloudlets_policy_activation" "pr_staging_latest" {
  policy_id = resource.akamai_cloudlets_policy.phased_release.id
  network   = "staging"
  version   = var.staging_version == null ? split(":", data.akamai_cloudlets_policy.example.id)[1] : var.staging_version
  # version               = resource.akamai_cloudlets_policy.phased_release.version
  associated_properties = var.hostnames
}

# now activate the latest version by terraform on staging.
resource "akamai_cloudlets_policy_activation" "pr_prod" {
  count     = var.activate_on_prod ? 1 : 0
  policy_id = resource.akamai_cloudlets_policy.phased_release.id
  network   = "production"
  version   = var.prod_version == null ? split(":", data.akamai_cloudlets_policy.example.id)[1] : var.prod_version
  # version               = resource.akamai_cloudlets_policy.phased_release.version
  associated_properties = var.hostnames
}
